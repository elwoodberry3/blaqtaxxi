...
3. **Spec numbers.** For each case S1–S12, L1–L2, V1, P1–P7, C1–C7: does a test exist, does it assert the *exact* numbers in the spec, and does it pass? Run `npm test` and cite the output. A test that only asserts `ok: true` where the spec gives a time is weak; flag it.
